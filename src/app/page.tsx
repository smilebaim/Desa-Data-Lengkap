  ...
  return (
    <TooltipProvider delayDuration={0}>
      <div className="relative h-screen w-screen overflow-hidden bg-slate-950 text-white font-body">
        ...
        {/* Slim Floating Dock (Bottom) */}
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 w-full max-w-lg px-6 flex flex-col items-center gap-4">
          <div className="flex flex-col items-center gap-2 mb-2">
            <span className="text-[9px] text-white/40 font-bold uppercase tracking-[0.4em]">Integrated Network</span>
          </div>
          ...
          <nav className="flex items-center justify-center gap-1.5 p-1.5 bg-slate-950/50 backdrop-blur-2xl border border-white/10 rounded-full shadow-2xl ring-1 ring-white/5">
            ...
            {bottomMenus.length === 0 && (
              <div className="py-2 px-8">
                <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest italic">No Items Configured</p>
              </div>
            )}
          </nav>
        </div>
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <Button variant="ghost" className="flex flex-col items-center justify-center h-12 w-16 gap-1 rounded-full text-slate-400 hover:bg-primary/20 hover:text-white transition-all duration-300 group">
          ...
          <span className="text-[8px] font-bold uppercase tracking-tighter opacity-40 group-hover:opacity-100">
            {label}
          </span>
        </Button>
      </TooltipTrigger>
