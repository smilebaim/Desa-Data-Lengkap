  ...
  return (
    <TooltipProvider delayDuration={0}>
      <div className="relative h-screen w-screen overflow-hidden bg-slate-950 text-white font-body">
        ...
        <header className="absolute top-4 sm:top-6 left-1/2 -translate-x-1/2 z-20 w-full max-w-5xl px-3 sm:px-4 pointer-events-none">
          <div className="flex items-center justify-between gap-2 sm:gap-4 pointer-events-auto bg-slate-950/40 backdrop-blur-md border border-white/5 p-1 sm:p-1.5 rounded-full shadow-2xl ring-1 ring-white/5">
        ...
        {/* Ultra-Slim Left Toolbar - Optimized for Mobile */}
          ...
          <div className="flex flex-col gap-1 p-1 bg-slate-950/40 backdrop-blur-md border border-white/5 rounded-full shadow-2xl ring-1 ring-white/5">
            ...
            {leftMenus.length === 0 && (
              <ToolbarButton tooltip="Map Tools">
                <Layers className="h-3.5 w-3.5 sm:h-4 sm:w-4 opacity-40" />
              </ToolbarButton>
            )}
          </div>
        ...
        {/* Ultra-Slim Floating Dock (Bottom) - Responsive & Scrollable */}
        <div className="absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-20 w-full max-w-[95vw] sm:max-w-md px-4 flex flex-col items-center gap-2">
          <div className="flex items-center gap-2">
            <div className="h-[1px] w-3 sm:w-4 bg-white/10" />
            <span className="text-[6px] sm:text-[7px] text-white/20 font-bold uppercase tracking-[0.3em] sm:tracking-[0.4em] whitespace-nowrap">Indonesian Village Network</span>
            <div className="h-[1px] w-3 sm:w-4 bg-white/10" />
          </div>
          
          <nav className="flex items-center justify-start sm:justify-center gap-1 p-1.5 bg-slate-950/50 backdrop-blur-2xl border border-white/10 rounded-full shadow-2xl ring-1 ring-white/5 max-w-full overflow-x-auto no-scrollbar">
            ...
            {bottomMenus.length === 0 && (
              <div className="py-2 px-6 sm:px-10">
                <p className="text-[8px] sm:text-[9px] text-slate-500 font-bold uppercase tracking-widest italic whitespace-nowrap">No Items</p>
              </div>
            )}
          </nav>
        </div>

        {/* Minimalist Zoom Controls - Adjusted for Mobile */}
        <div className="absolute bottom-4 sm:bottom-6 right-3 sm:right-6 z-20 flex flex-col gap-1.5">
          <button className="h-7 w-7 sm:h-8 sm:w-8 bg-slate-950/40 backdrop-blur-md border border-white/5 rounded-full flex items-center justify-center hover:bg-primary transition-all group shadow-xl ring-1 ring-white/5">
            <Plus className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-white/60 group-hover:text-white" />
          </button>
          <button className="h-7 w-7 sm:h-8 sm:w-8 bg-slate-950/40 backdrop-blur-md border border-white/5 rounded-full flex items-center justify-center hover:bg-primary transition-all group shadow-xl ring-1 ring-white/5">
            <Minus className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-white/60 group-hover:text-white" />
          </button>
        </div>
      </div>
    </TooltipProvider>
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <Button variant="ghost" size="icon" className="h-7 w-7 sm:h-8 sm:w-8 text-slate-400 hover:bg-primary hover:text-white rounded-full transition-all duration-300">
          {children}
        </Button>
      </TooltipTrigger>
...
function NavButton({ children, label }: { children: React.ReactNode, label: string }) {
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <Button variant="ghost" className="flex flex-col items-center justify-center h-10 w-12 sm:h-12 sm:w-16 gap-0.5 sm:gap-1 rounded-full text-slate-400 hover:bg-primary/20 hover:text-white transition-all duration-300 group shrink-0">
          <div className="transition-transform group-hover:scale-110">
            {children}
          </div>
          <span className="text-[7px] sm:text-[8px] font-bold uppercase tracking-tight opacity-50 group-hover:opacity-100 truncate w-full px-1 text-center">
            {label}
          </span>
        </Button>
      </TooltipTrigger>
