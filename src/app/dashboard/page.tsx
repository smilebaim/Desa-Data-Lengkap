...
export default function DashboardPage() {
  ...
  const seedDemoData = async () => {
    ...
    try {
      ...
      const sukmajuId = await addDoc(villagesRef, { 
        name: 'Desa Sukamaju', 
        province: 'Jawa Barat', 
        population: 4500, 
        area: 12.5, 
      ...
      const mekarsariId = await addDoc(villagesRef, { 
        name: 'Desa Mekarsari', 
        province: 'Bali', 
        population: 3200, 
        area: 8.2, 
      ...
      const pProfile = await addDoc(pagesRef, { 
        title: 'Laporan Pembangunan 2024', 
      ...
      await addDoc(menusRef, { label: 'Informasi Publik', icon: 'BarChart', href: `/p/${pProfile.id}`, order: 1, position: 'bottom' });
      await addDoc(menusRef, { label: 'Peta Statistik', icon: 'PieChart', href: `/visualizations`, order: 2, position: 'bottom' });
      await addDoc(menusRef, { label: 'Bantuan', icon: 'Info', href: `/p/${pProfile.id}`, order: 3, position: 'header' });
    ...
    <div className="space-y-8 text-left">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="border-none shadow-xl rounded-[2rem]">
          ...
          <CardContent>
            <div className="text-3xl font-black text-slate-900 text-left">Online</div>
        ...
        <Card className="border-none shadow-xl rounded-[2rem]">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-bold uppercase text-slate-500 tracking-wider">Metrik Aktif</CardTitle>
            <MenuIcon className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-slate-900 text-left">Geospasial</div>
        ...
        <Card className="border-none shadow-xl rounded-[2rem]">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-bold uppercase text-slate-500 tracking-wider">Potensi Desa</CardTitle>
            <MapIcon className="h-4 w-4 text-primary" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-slate-900 text-left">IDM & Anggaran</div>
            <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase">Mendukung Analisis AI</p>
          </CardContent>
        </Card>
      </div>

      <Card className="border-none shadow-2xl rounded-[3rem] bg-slate-900 text-white p-8 relative overflow-hidden text-left">
        ...
        <CardContent className="px-0 pt-6 grid md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div className="flex gap-4">
              <div className="h-8 w-8 bg-primary rounded-lg flex items-center justify-center font-bold text-white shrink-0">1</div>
              <p className="text-sm text-slate-300">Buat grafik di <strong>Statistik & Data</strong> menggunakan metrik IDM atau Anggaran.</p>
            </div>
            <div className="flex gap-4">
              <div className="h-8 w-8 bg-primary rounded-lg flex items-center justify-center font-bold text-white shrink-0">2</div>
              <p className="text-sm text-slate-300">Sematkan kode diagram di <strong>Manajemen Halaman</strong> untuk laporan otomatis.</p>
            </div>
          </div>
          <div className="space-y-4">
            <div className="flex gap-4">
              <div className="h-8 w-8 bg-primary rounded-lg flex items-center justify-center font-bold text-white shrink-0">3</div>
              <p className="text-sm text-slate-300">Gambar aset kategori INFRA, FASUM, atau SDA di <strong>Editor Spasial</strong>.</p>
            </div>
            <div className="flex gap-4">
              <div className="h-8 w-8 bg-primary rounded-lg flex items-center justify-center font-bold text-white shrink-0">4</div>
